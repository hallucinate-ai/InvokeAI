...
async function requestModelChange(model)
	let output = {
		"model_name": model,
		"model_list":{
...
async function intermediateResult(sid){
	let output = {
...
async function progressUpdate(sid){
	let output = {
		"currentStep": 1,
		"totalSteps": 50,
		"currentIteration": 1,
...
async function requestImages(user){
	let output ={}
	return output
}
...
async function generateImage(sid){
...
async function galleryImages(sid){
	let output = {
		"images": [
			{
...
async function startRestServer(){
	...
	app.get('/socket.io/', (req, res) => {
		...
		&transport=polling&t=OP042ev
		if (req.query.EIO == 4 && transport == 'polling' && t != null && sid == null){
			// convert the timestamp t to a hashed string named sid with length 20
			let sid = sha1(t).substring(0,20)
			let response = [{"sid":sid,"upgrades":["websocket"],"pingTimeout":60000,"pingInterval":100000}]
			res.send(response)
		}
		else if (req.query.EIO == 4 && transport == 'polling' && t != null && sid != null){
			let response = "OK"
			res.send(response)
			// exit express and return to the main function
			return
		}
		else if (req.query.EIO == 4 && transport == 'websocket' && sid != null){
