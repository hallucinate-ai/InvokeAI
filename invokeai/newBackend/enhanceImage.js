...
function enhance(request, request2, request3, timestamp, task, context, socket){
	let cwd = process.cwd()
...
export function main(request, request2, request3, timestamp, socket){
	...
	if(request2 != false  ){
		...
		context = fs.readFileSync(cwd + '/gallery/defaultUser/' + timestamp + '.png')
		let results = enhance(request, request2, request3, timestamp, task, context, socket)
	} else if(request3 != false && request2 == false){
		let type = request3["type"]
		if (type == "codeformer"){
			task = {
				"command": "enhance_face",
